...
public class DemoDBManager {
    ...
    synchronized public void saveContactList(List<User> contactList) {
        ...
        if (db.isOpen()) {
            ...
            for (User user : contactList) {
                ...
                if(user.getLng() != -1)
                    values.put(UserDao.COLUMN_NAME_LNG, user.getLng());
                if(user.getLat() != -1)
                    values.put(UserDao.COLUMN_NAME_LAT, user.getLat());
                if(user.getUser_id() != -1)
                    values.put(UserDao.COLUMN_NAME_USER_ID, user.getUser_id());
    ...
    synchronized public Map<String, User> getContactList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                int sex=cursor.getInt(cursor.getColumnIndex(UserDao.COLUMN_NAME_SEX));
                double lng=cursor.getDouble(cursor.getColumnIndex(UserDao.COLUMN_NAME_LNG));
                double lat=cursor.getDouble(cursor.getColumnIndex(UserDao.COLUMN_NAME_LAT));
                int user_id=cursor.getInt(cursor.getColumnIndex(UserDao.COLUMN_NAME_USER_ID));
                User user = new User();
    ...
    public synchronized Integer saveMessage(InviteMessage message){
        ...
        if(db.isOpen()){
            ...
            values.put(InviteMessgeDao.COLUMN_NAME_SEX,message.getSex());
            values.put(InviteMessgeDao.COLUMN_NAME_LNG,message.getLng());
            values.put(InviteMessgeDao.COLUMN_NAME_LAT,message.getLat());
            values.put(InviteMessgeDao.COLUMN_NAME_NICK,message.getNick());
            values.put(InviteMessgeDao.COLUMN_NAME_USER_ID,message.getUser_id());
    ...
    synchronized public List<InviteMessage> getMessagesList(){
        ...
        if(db.isOpen()){
            ...
            while(cursor.moveToNext()){
                ...
                int sex=cursor.getInt(cursor.getColumnIndex(InviteMessgeDao.COLUMN_NAME_SEX));
                String avar=cursor.getString(cursor.getColumnIndex(InviteMessgeDao.COLUMN_NAME_AVATAR));
                Double lng=cursor.getDouble(cursor.getColumnIndex(InviteMessgeDao.COLUMN_NAME_LNG));
                Double lat=cursor.getDouble(cursor.getColumnIndex(InviteMessgeDao.COLUMN_NAME_LAT));
                String nick=cursor.getString(cursor.getColumnIndex(InviteMessgeDao.COLUMN_NAME_NICK));
                int user_id=cursor.getInt(cursor.getColumnIndex(InviteMessgeDao.COLUMN_NAME_USER_ID));
                msg.setId(id);
                msg.setFrom(from);
