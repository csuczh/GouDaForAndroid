...
public class DogTypeEditActivity extends Activity implements OnClickListener{

	public static int MAX_LEN = 8;

	public static String DOGVARITY = "DOGVARITY";
	...
	public void onClick(View v) {
		...
		case R.id.tv_job_complete:
			Toast.makeText(DogTypeEditActivity.this,et_job.getText().toString(),Toast.LENGTH_SHORT).show();
			UpdateJob();
			break;
		}
	}
	...
	private void UpdateJob() {
		try{
			...
			if(et_job.getText()!=null && !"".equals(et_job.getText())){
				...
				intent.putExtra(DOGVARITY, et_job.getText().toString());
				setResult(RESULT_OK, intent);
				finish();
			}else{
				et_job.setError("品种不能为空");
			}
