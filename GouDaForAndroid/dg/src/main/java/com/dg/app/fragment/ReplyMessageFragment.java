...
public class ReplyMessageFragment extends  BaseListFragment<Message>
{
    private DGUser user;

    protected static final String TAG = ReplyMessageFragment.class.getSimpleName();

    private static final String CACHE_KEY_PREFIX = "reply_messages_list_";

    @Override
    protected MessagesAdapter getListAdapter() {
        return new MessagesAdapter(getActivity(),MessagesAdapter.REPLY_MESSAGE);
    }
    ...
    protected void sendRequestData() {
        ...
        if(user!=null&&user.getUserid()!=0){
            DGApi.replyMessages(user.getUserid(), "", mHandler);
        } else {
