...
@SuppressLint("DgApi")
public abstract class MomentsBaseListFragment<T extends Entity> extends BaseFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (args != null) {
                ...
                user_id = args.getInt(BUNDLE_KEY_USER_ID, 0);
                cityid = args.getInt(BUNDLE_KEY_CITY_ID, 0);
                page=args.getInt(BUNDLE_KEY_PAGE,0);
                num=args.getInt(BUNDLE_KEY_NUM,10);
            }
        }catch (Exception ex)
        {
            ex.printStackTrace();
    ...
    protected void executeOnLoadDataSuccess(List<T> data) {
        try {
            ...
            if ((mAdapter.getCount() + data.size()) == 0) {
                ...
                AppContext.showToast(data.size()+"STATE_NO_MORE");
                adapterState = ListBaseAdapter.STATE_NO_MORE;
                mAdapter.notifyDataSetChanged();
            } else {
                AppContext.showToast(data.size()+"STATE_LOAD_MORE");
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (mState == STATE_NONE && scrollEnd) {
            AppContext.showToast("准备加载数据");
            if (mAdapter.getState() == ListBaseAdapter.STATE_LOAD_MORE
                    || mAdapter.getState() == ListBaseAdapter.STATE_NETWORK_ERROR) {
                mCurrentPage++;
                mState = STATE_LOADMORE;
                requestData(false);
                AppContext.showToast("加载数据："+mCurrentPage+"页");
