...
import com.dg.app.fragment.DogKnowMoreFragment;
import com.dg.app.fragment.FindNearbyFragment;
import com.dg.app.ui.SearchArticleActivity;
...
@SuppressLint("DgApi")
public abstract class DogKnowBaseListFragment extends BaseFragment
        implements SwipeRefreshLayout.OnRefreshListener, OnItemClickListener,
        OnScrollListener {

    public static final String BUNDLE_KEY_CATALOG = "BUNDLE_KEY_CATALOG";

    public static final String BUNDLE_KEY_DEDVICE="BUNDLE_KEY_DEDVICE";
    ...
    @InjectView(R.id.dogknow_more)
    protected FrameLayout moreLayout;
    @InjectView(R.id.listview)
    protected ListView mListView;

    protected ListBaseAdapter<Question> mAdapter;
    ...
    private AsyncTask<String, Void, ListEntity<Question>> mCacheTask;
    ...
    protected int getLayoutId() {
        return R.layout.dogknow_fragment_pull_refresh_listview;
    }
    @Override
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (args != null) {
                ...
                device = args.getString(BUNDLE_KEY_DEDVICE, "android");
                city_id = args.getInt(BUNDLE_KEY_CITY_ID, 8);
                Toast.makeText(getActivity(), device + " " + city_id, Toast.LENGTH_LONG).show();
    ...
    public void initView(View view) {
        ...
        left = (Button) view.findViewById(R.id.button_dogknow_recommond);
        right = (Button) view.findViewById(R.id.button_dogknow_more);
        ...
        right.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    DogKnowMoreFragment dogKnowMoreFragment = new DogKnowMoreFragment();
                    //fragment管理器

                    fm=getFragmentManager();
                    FragmentTransaction transaction2 = fm.beginTransaction();
                    transaction2.replace(R.id.dogknow_more, dogKnowMoreFragment);
                    transaction2.commit();
                }catch (Exception ex)
                {
                    ex.printStackTrace();
    ...
    protected abstract ListBaseAdapter<Question> getListAdapter();
    ...
    protected ListEntity<Question> parseList(InputStream is) throws Exception {
        return null;
    }

    protected ListEntity<Question> readList(Serializable seri) {
        return null;
    }
    ...
    private class CacheTask extends AsyncTask<String, Void, ListEntity<Question>> {
        ...
        @Override
        protected ListEntity<Question> doInBackground(String... params) {
        ...
        @Override
        protected void onPostExecute(ListEntity<Question> list) {
            super.onPostExecute(list);
            if (list != null) {
                System.out.println("cache-------------->"+list.getList().get(0).getHot());
    ...
    protected void executeOnLoadDataSuccess(List<Question> data) {
            ArrayList<Question> topQuestionList=new ArrayList<>();
            ArrayList<Question> hotQuestionList=new ArrayList<>();

            for(int i=0;i<data.size();i++)
            {
                     if(data.get(i).getHot()==2)
                     {
                         topQuestionList.add(data.get(i));
                     }
                     if(data.get(i).getHot()==1)
                     {
                         hotQuestionList.add(data.get(i));
                     }
                System.out.print("data------------>"+data.get(i).getHot()+"~~~~~~~");
            }
             System.out.println();
            if(topQuestionList.size()>0)
               fillUI(topQuestionList);
            ...
            for (int i = 0; i < hotQuestionList.size(); i++) {
                if (compareTo(mAdapter.getData(), hotQuestionList.get(i))) {
                    hotQuestionList.remove(i);
                    i--;
                }
            }
            int adapterState = ListBaseAdapter.STATE_EMPTY_ITEM;
            if ((mAdapter.getCount() + hotQuestionList.size()) == 0) {
                adapterState = ListBaseAdapter.STATE_EMPTY_ITEM;
            } else if (hotQuestionList.size() == 0
                    || (hotQuestionList.size() < getPageSize() && mCurrentPage == 0)) {
            ...
            mAdapter.addData(hotQuestionList);
    ...
    class ParserTask extends AsyncTask<Void, Void, String> {
        ...
        //保存数据
        public void saveCacheData(ListEntity<Question> data)
        {
        ...
        protected String doInBackground(Void... params) {
            try {
                ListEntity<Question> data = parseList(new ByteArrayInputStream(
                        reponseData));
...
//    }
    ...
    protected  abstract  void fillUI(ArrayList<Question> questions);
}
