...
public class FollowedMomentFragment extends MomentsBaseListFragment<Moment>
{

    private DGUser user;

    protected static final String TAG = FollowedMomentFragment.class.getSimpleName();

    private static final String CACHE_KEY_PREFIX = "followed_moments_list_";

    @Override
    protected MomentsAdapter getListAdapter() {
        return new MomentsAdapter(getActivity());
    }
    ...
    protected void sendRequestData() {
        ...
        if(user!=null&&user.getUserid()!=0) {
            DGApi.followedMoments(user.getUserid(), mCurrentPage, MomentsBaseListFragment.PAGE_SIZE, mHandler);
        }else{
