...
public class LiuDogInvitationsAdapter extends ListBaseAdapter<LiuGouInvitation>{

    private Activity context;

    public LiuDogInvitationsAdapter(Activity context) {
    ...
    protected View getRealView(final int position, View convertView, ViewGroup parent) {
        ...
        try {
            ...
            final LiuGouInvitation liuGouInvitation = mDatas.get(position);
            /**
             * 点击单项状态进入邀请详情界面
             */
...
//                    context.startActivity(intent);
             ...
             * 1、设置时间
             */
//            vh.tv_time.setText(StringUtils.friendlyTime(liuGouInvitation.getPublish_time()));
            vh.tv_time.setText(liuGouInvitation.getPublish_time());
            /**
             *2、设置请求消息
             */

            vh.tv_require_info.setText(liuGouInvitation.getInvite_explain());
                ...
                public void onClick(View v) {
                    ...
                    dgDialogBuilder.setMessage("确定删除吗?").setNegativeButton("取消", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if(user_id!=0){
                                DGApi.deleteInvitation(user_id, liuGouInvitation.getWalk_id(), InvitationFragment.INVITATION_TYPE[0], new AsyncHttpResponseHandler() {
                                    @Override
